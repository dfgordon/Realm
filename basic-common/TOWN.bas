0 FO% = FL%: FL% = 0: XT = 2: YT = 20
1 WY% = 0: WX% = 0: T$ = B$
5  POKE 8185,1: POKE 8174,82: POKE 8175,3
6  POKE 8176,40: POKE 8177,40: POKE 8181,4: POKE 8182,2+8*16
10  HGR: HOME : GOTO 100

20  GOSUB 150: VTAB 20: GET A$: PRINT: AE = AE - .25: IF AE < 0 THEN 50000
30  IF AE < 100 AND FS% = 0 THEN FS% = -1
40  GOSUB 130
71  IF XT < 2 OR XT > 37 OR YT < 2 OR YT > 37 THEN 8000
73  IF A$ = "Z" THEN 4000
74  IF A$ = "B" THEN 500
75  IF A$ = "X" THEN 600
76  IF A$ = "C" THEN 700
78  IF A$ = "R" THEN 2800
81  IF A$ = "T" THEN 7400
82  IF A$ = "U" THEN 1000
83  IF A$ = "S" THEN 1100
84  IF A$ = "F" THEN HOME: FS% = -ABS(FS%) - 1: IF FS%<-3 THEN FS% = 0
85  IF A$ = "I" THEN 2000
86  IF A$ = "A" THEN 200
87  IF A$ = "Q" OR A$ = CHR$(18) THEN 8070
95  IF A$ = CHR$(16) THEN A = RND(1): HOME: VTAB 21: PRINT "DISCARDING ONE FATE CARD";: GET A$: HOME
99  IF A$ = CHR$(19) THEN 7050
100  POKE 8186,WX%: POKE 8187,FL%: POKE 8190,XT: POKE 8191,YT: CALL 7545: IF  PEEK (6) = 255 THEN 114
104  CALL 7486: IF  PEEK (6) = 12 THEN 13000
106  IF WX% = 0 THEN 20
108  A =  PEEK (7)
110  IF A = 0 OR A = 7 OR A = 8 THEN 250
112  GOTO 20
114  XT = XT - XK : YT = YT - YK: GOSUB 30070: GOTO 20

130  XK = 0: YK = 0
131  IF A$ =  CHR$ (11) OR A$ = "-" OR A$ = "8" OR A$ = "/" THEN YK = - 1
132  IF A$ =  CHR$ (8) OR A$ = "P" THEN XK = - 1
133  IF A$ =  CHR$ (21) OR A$ = "[" THEN XK = 1
134  IF A$ =  CHR$ (10) OR A$ = ";" OR A$ = "M" OR A$ = "." THEN YK = 1
135  IF A$ = "0" OR A$ = "7" THEN XK=-1: YK=-1
136  IF A$ = "=" OR A$ = "9" THEN XK=1: YK=-1
137  IF A$ = "L" OR A$ = "N" THEN XK=-1: YK=1
138  IF A$ = "'" OR A$ = "," THEN XK=1: YK=1
139  XT = XT + XK: YT = YT + YK: RETURN

140  POKE 8190,XT: POKE 8191,YT: CALL 7486: RETURN
141  POKE 8190,XT: POKE 8191,YT: POKE 6,N: POKE 7,255: CALL 7504: RETURN
142  POKE 8190,XT: POKE 8191,YT: POKE 6,255: POKE 7,N: CALL 7504: RETURN

150  IF FS% = 0 THEN RETURN
151  IF PEEK(1616) = 160 THEN FS% = -ABS(FS%)
152  IF AE<100 THEN INVERSE
153  VTAB 21: PRINT "FOOD": NORMAL: PRINT "    ";: HTAB 1: PRINT  INT (AE): IF FS% = -1 OR FS% > 0 THEN FS% = ABS(FS%): RETURN
154  FS% = ABS(FS%): PRINT "GOLD": PRINT AD%;
155  FOR I = 0 TO 3: VTAB 21 + I: HTAB 6
156  IF FS% = 2 THEN PRINT LEFT$(DW$(I),12);
157  IF FS% = 3 THEN PRINT LEFT$(DV$(I),12);
158  A$ = STR$(AB%(I))+"/"+STR$(AC%(I)): HTAB 19: PRINT LEFT$(DX$(I),21-LEN(A$));: HTAB 41-LEN(A$): POKE 35,25: PRINT A$;: POKE 35,24: NEXT
159  RETURN

190  HOME: VTAB 21: PRINT A$;" (#) ";
191  GET A$: IF A$ = CHR$(27) THEN POP: HOME: GOTO 20
192  A = VAL(A$) - 1: IF A<0 OR A>3 THEN 191
193  IF AB%(A) = 0 THEN POP: GOTO 2920
194  RETURN

199  GOSUB 30050: HOME: GOTO 100

200  IF  FL% = 16 THEN 7599
210  HOME: VTAB 21: PRINT "ATTACK DIRECTION: ";: GET A$
220  GOSUB 130: GOSUB 140: A = PEEK(7)
230  IF  A > 13 THEN XT = XT - XK: YT = YT - YK: GOTO 7599
240  HOME: WX% = 1: POKE 8186,WX%: CALL 7545: IF PEEK(6) = 255 THEN 114
250  GOSUB 370: GC$ = "": GD$ = "": GI$ = "": A = A + 1
260  ON A GOTO 300,310,310,310,310,320,330,340,350,310,310,310,310,350

300 GA = 9:GB = 24:GE = 85: GM = 4: GOTO 360
310 GA = 2:GB = 10:GE = 70: GM = 1: GOTO 360
320 GA = 6:GB = 20:GE = 80: GM = 4: GOTO 360
330 GA = 9:GB = 20:GE = 90: GM = 4: GOSUB 380: GOTO 360
340 GA = 13:GB = 25:GE = 90: GM = 6: GC$ = "BALL" + "ISTA": GD$ = "": GOTO 360
350 GA = 15:GB = 30:GE = 90: GM = 4: GC$ = "FIRE " + "BREATH": GD$ =  LEFT$ (GC$,1): GOTO 360

360  FOR I = 1 TO GM: GN(I) =  GA*(INT (3 *  RND (1)) + 9): NEXT I
361  HOME : PG$ = "COM" + "BAT": WT% = A - 1: DG% = 0
362  IF WT%<9 THEN N = 15: GOSUB 142
363  IF WT%>8 THEN N = 14: GOSUB 142
364  POKE 216,0: POKE 4096,0: POKE 103,1: POKE 104,16: GOTO 10

370  IF A = 0 THEN GO$ = "GUA"+"RD": RETURN
371  IF A = 1 OR A = 9 THEN GO$ = "HUM"+"AN": RETURN
372  IF A = 2 OR A = 10 THEN GO$ = "EL"+"F": RETURN
373  IF A = 3 OR A = 11 THEN GO$ = "DWA"+"RF": RETURN
374  IF A = 4 OR A = 12 THEN GO$ = "HOB"+"BIT": RETURN
375  IF A = 5 THEN GO$ = "ADV"+"ENTURER": RETURN
376  IF A = 6 THEN GO$ = "WIZ"+"ARD": RETURN
377  IF A = 7 THEN GO$ = "MAR"+"INE": RETURN
378  IF A = 8 OR A = 13 THEN GO$ = "DRA"+"GON": RETURN

380  M =  INT (3 *  RND (1))
381  IF M = 0 THEN GF = 20:GH = 4:GI$ = "PHANTOM " + "ARROW": RETURN
382  IF M = 1 THEN GF = 40:GH = 1:GI$ = "FIRE" + "BALL": RETURN
383  GF =  - 4:GH = 1:GI$ = "PARA" + "LYZE": RETURN

500  GOSUB 140: IF  PEEK (6) <> 11 THEN 8070
510  IF  PEEK (7) = 7 AND WY% = 0 THEN HOME: VTAB 21: PRINT "BLOCKED BY MARINE. ";: GOTO 199
520  N = 4: GOSUB 141: N = 15: GOSUB 142: FL% = 16: GOTO 100

600  GOSUB 140
610  IF  FL% <> 16 OR PEEK(6) <> 4 OR PEEK(7) <> 15 THEN 8070
620  FL% = 0: N = 11: GOSUB 141: GOTO 100

700  A$ = "WHO WILL CAST": GOSUB 190
710 B$ = DX$(A)
720  IF B$ = "RAISE DEAD" THEN 750
721  IF B$ = "HEAL 1" THEN 760
722  IF B$ = "HEAL 2" THEN 765
723  IF B$ = "HEAL 3" THEN 770
724  IF B$ = "KNOCK" OR B$ = "PASSWALL" THEN 800
730  GOTO 2910

750  HOME: VTAB 21: PRINT "RAISE WHOM (#) ";: GET A$
751 B =  VAL (A$) - 1: IF B < 0 OR B > 3 THEN 750
752  IF AB%(B) > 0 THEN HOME: VTAB 21: PRINT NM$(B);" IS NOT DEAD! ";: GOTO 199
753  IF B$ <> "RAISE DEAD" THEN GOSUB 2200
754  IF B$ = "RAISE DEAD" THEN GOSUB 2300
755 AB%(B) = 10: HOME : VTAB 21: PRINT "-CASTING RAISE DEAD-": GOSUB 30000
756  PRINT NM$(B);" IS RAISED! ";: GET A$: HOME : GOTO 100

760 CA = 50: GOTO 780
765 CA = 100: GOTO 780
770 CA = 200
780 HOME: VTAB 21: PRINT "HEAL WHOM (#) ";: GET A$
781 B =  VAL (A$) - 1: IF B < 0 OR B > 3 THEN 780
782  IF AB%(B) = 0 THEN 2920
783  IF LEFT$(B$,2) <> "HE" THEN GOSUB 2200
784  IF LEFT$(B$,2) = "HE" THEN GOSUB 2300
785  HOME : VTAB 21: PRINT "-CASTING HEAL-": GOSUB 30000
786 C =  INT (CA *  RND (1)) + CA
787 AB%(B) = AB%(B) + C: IF AB%(B) > AC%(B) THEN AB%(B) = AC%(B)
788  PRINT C;" POINTS HEALED. ";: GET A$: HOME : GOTO 100

800  GOSUB 850: IF B$ = "KNOCK" AND PEEK(6) = 10 THEN 815
810  IF B$ = "PASSWALL" AND (PEEK(6) = 5 OR PEEK(6) = 10) THEN 815
812  HOME: VTAB 21: PRINT "WRONG TARGET.";: GOTO 199
815  GOSUB 2300
816  HOME: VTAB 21: PRINT "-CASTING ";B$;"-": GOSUB 30000
830  N = 13: XT=XT+XK: YT=YT+YK: GOSUB 141: GOSUB 880: HOME: GOTO 100

850  HOME : VTAB 21: PRINT "WHICH DIRECTION: ";: GET A$: PRINT: GOSUB 130: GOSUB 140
880 XT = XT - XK: YT = YT - YK: RETURN

1000  A$ = "WHO WILL USE": GOSUB 190
1005 B$ = DW$(A)
1010  IF B$ = "POTION OF PASSAGE" THEN 1050
1013  IF B$ = "POTION OF VISION" THEN 1070
1015  IF B$ = "ROD OF RESURRECTION" THEN 750
1016  IF B$ = "POTION OF HEALING" THEN 760
1020  IF B$ = "AMULET OF HEALING" THEN 770
1021  IF  LEFT$ (B$,3) = "GEM" THEN 1030
1023  GOTO 2900

1030  IF XT = 31 AND YT = 19 AND LEFT$(T$,2) = "IN" THEN 1035
1031  GOTO 8070
1035  HOME : VTAB 21: PRINT "-ENLIGHTENING-": GOSUB 30000: GOSUB 30090
1036  IF AG%(A) > 49 THEN 1040
1037  PRINT NM$(A);" COULD NOT": PRINT "COMPREHEND AND IS DEAD! ";:AB%(A) = 0: GET A$: HOME : GOTO 100
1040  FG%(3)=16: PRINT NA$;" ARE ENLIGHTENED! ";: GET A$: HOME : GOTO 100

1050  GOSUB 850: IF PEEK(6)=5 OR PEEK(6)=10 THEN 1060
1056  GOTO 812
1060  GOSUB 2200: GOSUB 1099: GOTO 830

1070  IF PEEK(8181) = 6 THEN HOME: VTAB 21: PRINT "REACHED LIMIT. ";: GOTO 199
1071  GOSUB 2200: GOSUB 1099: I = PEEK(8181)+1: POKE 8181,I: POKE 8182,262-33*I: HOME: GOTO 100

1099  HOME: VTAB 21: PRINT "-USING ";B$;"-": GOSUB 30000: RETURN

1100  HOME: VTAB 21: GOSUB 140: M = PEEK(6): IF M <>0 AND M <> 9 AND M <> 14 THEN PRINT "NOTHING TO STEAL. ";: GOTO 199
1101  HOME: VTAB 21: PRINT "CHARACTER TO ATTEMPT THEFT (#) ";: GET A$: A =  VAL (A$) - 1: IF A<0 OR A>3 THEN 1101
1102  IF AB%(A) = 0 THEN 1101
1106 I = 0: IF RA$(A) = "HOBBIT" THEN I = I + 15
1107 I = I + AI%(A) + AG%(A) / 3
1108  IF CL$(A) = "THIEF" THEN I = I + 50
1109  IF CL$(A) = "MONK" THEN I = I + 30
1110  IF I > 95 THEN I = 95
1115  IF I < 99 * RND(1) OR WX% = 1 THEN HOME: VTAB 21: PRINT "UNSUCCESSFUL! ";: GOSUB 30050: WX% = 1: HOME: GOTO 100
1120  HOME: VTAB 21: PRINT "SUCCESSFUL!!": GOSUB 30060
1130  IF  M = 9 OR M = 14 THEN 1300

1200  PRINT "FOOD +50 ";:AE = AE + 50: IF AE > 9999 THEN AE = 9999
1210  GET A$: HOME: GOTO 100

1300  C = 50: I = 99 * RND(1): IF I>75 THEN C = 100
1310  PRINT "YOU GOT ";C;" GOLD ";: AD% = AD% + C: IF AD%>9999 THEN AD%=9999
1320  GOTO 1210

2000  TEXT: A = 0: B$ = "FIRST": GOSUB 2080: IF M=999 THEN 2040
2010  B = A: C = M: A = 0: B$ = "SECOND": GOSUB 2080: IF M=999 THEN 2040
2020 B$ = AP$(A,M): AP$(A,M) = AP$(B,C): AP$(B,C) = B$
2030  GOSUB 7100: A = B: GOSUB 7100
2040  POKE  - 16304,0: HOME : GOTO 100
2080  HOME: PRINT "--INVENTORY SWAP--":PRINT:PRINT B$;" ITEM (";NM$(A);")": GOSUB 4210: M = 0: N = 0: P = 7: I = 5: GOSUB 4450
2082  IF M = 99 THEN 2080
2083  RETURN

2200  P = ZA%(A) / 1000 + AH%(A): IF P/(10+P) >  RND (1) THEN RETURN
2202  AP$(A,DR%(A,0)) = ""
2204  P = 8: FOR I = 7 TO 0 STEP -1: P = P*(AP$(A,I)<>DW$(A)) + I*(AP$(A,I)=DW$(A)): NEXT: DR%(A,0) = P: IF P<8 THEN RETURN
2206  HOME: VTAB 21: INVERSE: PRINT DW$(A);" IS EXPENDED ": NORMAL: DW$(A) = AP$(A,P): GOSUB 30100: RETURN

2300  P = ZA%(A) / 1000 + AH%(A): IF P/(10+P) >  RND (1) THEN RETURN
2302  AX$(A,DR%(A,2)) = ""
2304  P = 16: FOR I = 15 TO 0 STEP -1: P = P*(AX$(A,I)<>DX$(A)) + I*(AX$(A,I)=DX$(A)): NEXT: DR%(A,2) = P: IF P<16 THEN RETURN
2306  HOME: VTAB 21: INVERSE: PRINT DX$(A);" IS EXHAUSTED ": NORMAL: DX$(A) = AX$(A,P): GOSUB 30100: RETURN

2800  A$ = "READY WHO": GOSUB 190
2802  HOME: VTAB 21: PRINT NM$(A);" (ARROWS,A,Z,ENT,ESC)": FOR C=0 TO 2: DT%(C) = DR%(A,C): NEXT: C = 0
2803  PRINT "ITEM:  ";CHR$(DT%(0)+65);") ";LEFT$(DW$(A),24)
2804  PRINT "ARMOR: ";CHR$(DT%(1)+65);") ";DV$(A)
2805  PRINT "SPELL: ";CHR$(DT%(2)+65);") ";DX$(A);
2806  L = DT%(C): P = 9: IF C = 2 THEN P = 16

2810  HTAB 8: VTAB C+22: IF C<2 THEN B$ = AP$(A,L)
2811  IF C = 2 THEN B$ = AX$(A,L)
2814  PRINT CHR$(L+65);") ";: INVERSE: PRINT LEFT$(B$,24);: NORMAL: CALL -868
2820  HTAB 1: VTAB 20: GET A$: IF A$ = CHR$(27) THEN HOME: GOTO 100
2821  IF A$ = CHR$(13) THEN 2830
2822  IF A$ = CHR$(11) OR A$ = "A" THEN N = -1: GOTO 2850
2823  IF A$ = CHR$(10) OR A$ = "Z" THEN N = 1: GOTO 2850
2825  N = 0: IF A$ = CHR$(8) THEN N = -1
2826  IF A$ = CHR$(21) THEN N = 1
2827  IF N = 0 THEN 2820
2828  GOSUB 2860: DT%(C) = L: GOTO 2810

2830  FOR C=0 TO 2: DR%(A,C) = DT%(C): NEXT: DW$(A)=AP$(A,DT%(0)): DV$(A)=AP$(A,DT%(1)): DX$(A)=AX$(A,DT%(2)): HOME: GOTO 100

2850  HTAB 11: VTAB C+22: PRINT B$;: C = C + N: IF C<0 THEN C = 2
2851  IF C>2 THEN C = 0
2852  GOTO 2806

2860  B$ = LEFT$(CL$(A),1)
2861  L = L + N: IF L<0 THEN L = P
2862  IF L>P THEN L = 0
2866  IF C <> 2 THEN I = LEN(AP$(A,L))
2867  IF C = 2 THEN I = LEN(AX$(A,L))
2868  IF I = 0 THEN 2861
2869  IF C = 2 THEN RETURN
2870  A$ = LEFT$(AP$(A,L),5)
2871  M = A$="SKIN" OR A$="LEATH" OR A$="CHAIN" OR A$="PLATE": M = (C=0 AND M=1) OR (C=1 AND M=0)
2873  M = M OR (B$="W" OR B$="T" OR B$="M") AND (A$="CHAIN" OR A$="PLATE")
2874  M = M OR (B$<>"C" AND (A$="ROD O" OR A$="AMULE" OR A$="LIGHT" OR A$="FIRE "))
2875  M = M OR (B$<>"W" AND (A$="WAND " OR A$="STAFF"))
2876  IF M THEN 2861
2877  RETURN

2900  HOME: VTAB 21: PRINT "INVALID ITEM! ";: GOTO 199
2910  HOME: VTAB 21: PRINT "INVALID SPELL! ";: GOTO 199
2920  HOME: VTAB 21: PRINT NM$(A);" IS DEAD! ";: GOTO 199

4000  TEXT: HOME: M = 0: P = 3: A = 0
4010  ON M+1 GOSUB 4100,4150,4210,4310
4020  VTAB 23: PRINT "UD-ARROWS/A/Z=PAGE, ESC=QUIT ": PRINT "LR-ARROWS=CHARACTER ";: GET A$: IF A$ = CHR$(27) THEN HOME: POKE -16304,0: GOTO 20
4030  GOSUB 4400: HOME: GOTO 4010
4100  PRINT "PARTY: ";NA$: GOSUB 4700: PRINT "GOLD COINS: ";AD%: PRINT "FOOD SUPPLY: "; INT (AE)
4110  PRINT: PRINT "ACHIEVEMENTS:": FOR I=8 TO 14: VTAB I: PRINT "<?>": NEXT
4111  IF FG%(0)=16 THEN VTAB 8: PRINT "FONKRAKIS"
4112  IF FG%(1)=16 THEN VTAB 9: PRINT "ABYSS"
4113  IF FG%(2)=16 THEN VTAB 10: PRINT "WORNOTH"
4114  IF FG%(3)>0 THEN VTAB 12: PRINT "ENLIGHTENED"
4115  IF FG%(4)>0 THEN VTAB 11: PRINT "BLESSED"
4116  IF FG%(5)>0 THEN VTAB 13: PRINT "ETHEREAL"
4117  IF FG%(5)=32 THEN VTAB 14: PRINT "VANQUISHER"
4120  RETURN
4150  PRINT "NAME: ";NM$(A): PRINT "CLASS: ";CL$(A): PRINT "RACE: ";RA$(A): PRINT "HIT POINTS: ";AB%(A);"/";AC%(A)
4160  PRINT "STRENGTH: ";AF%(A): PRINT "INTELLIGENCE: ";AG%(A): PRINT "WISDOM: ";AH%(A): PRINT "AGILITY: ";AI%(A): PRINT "STAMINA: ";AJ%(A): PRINT "CHARISMA: ";AK%(A)
4170  PRINT "EXPERIENCE: ";ZA%(A): PRINT "LEVEL: "; INT (ZA%(A) / 1000) + 1;"/";WT%(A)
4180  PRINT : PRINT "WEAPON READY: ";DW$(A): PRINT "ARMOUR READY: ";DV$(A): PRINT "SPELL READY: ";DX$(A)
4190  RETURN
4210  PRINT NM$(A);"'S ITEMS:": FOR I = 0 TO 7: PRINT I + 1;") ";: PRINT AP$(A,I): NEXT: RETURN
4310  PRINT NM$(A);"'S SPELLS:": FOR I = 0 TO 15: PRINT CHR$(65+I);") ";: PRINT AX$(A,I): NEXT: RETURN

4400  N = 99: IF A$ = CHR$(8) THEN A = A - 1: IF A<0 THEN A = 3
4410  IF A$ = CHR$(21) THEN A = A + 1: IF A>3 THEN A = 0
4420  IF A$ = "A" OR A$ = CHR$(11) THEN N = M: M = M - 1: IF M<0 THEN M = P
4430  IF A$ = "Z" OR A$ = CHR$(10) THEN N = M: M = M + 1: IF M>P THEN M = 0
4440  RETURN

4450  HTAB 1: VTAB I+9: PRINT "LR-ARROWS=CHARACTER, ENTER=PLACE": PRINT "UD-ARROWS/A/Z=SLOT, ESC=CANCEL"
4460  HTAB 4: VTAB N+I: PRINT AP$(A,N);" ": INVERSE: HTAB 4: VTAB M+I: PRINT AP$(A,M);" ": NORMAL
4461  HTAB 34: VTAB I+10: GET A$: IF A$ = CHR$(27) THEN M = 999: RETURN
4470  IF A$ = CHR$(13) THEN RETURN
4480  GOSUB 4400: IF N = 99 THEN M = 99: RETURN
4490  GOTO 4460

4600  TEXT: HOME: PRINT "PLACING: ";B$: PRINT: GOSUB 4210: M = 0: N = 0: P = 7: I = 4: GOSUB 4450: IF M = 999 THEN PRINT: PRINT "SALE ABORTED! ";: GET A$: RETURN
4610  IF M = 99 THEN 4600
4620  IF AP$(A,M) = "" THEN 4650
4630  HOME: PRINT "REPLACE ";AP$(A,M);" (Y/N) ? ";: GET A$: IF A$ <> "Y" AND A$ <> "N" THEN 4630
4640  IF A$ = "N" THEN 4600
4650  AP$(A,M) =  LEFT$ (B$ + " ", LEN (B$)): AD% = AD% - C: GOSUB 7100: RETURN

4700  PRINT T$;", ";RM$: RETURN

7000  REM INPUT
7002 B$ = "": HTAB 1: PRINT ">";: CALL -868
7004  GET CH$: CH = ASC(CH$): IF CH = 13 THEN RETURN
7008  IF CH = 8 OR CH = 127 THEN 7014
7010  IF CH<32 OR CH>126 OR CH=44 OR LEN(B$)>30 THEN 7004
7012  PRINT CH$;: B$ = B$ + CH$: GOTO 7004
7014  IF LEN (B$) < 2 THEN 7000
7016 B$ =  LEFT$ (B$, LEN (B$) - 1): POKE 36, PEEK (36) - 1: CALL  - 868: GOTO 7004

7050  HOME : VTAB 21: IF  PEEK (778) = 96 THEN 7080
7060  POKE 778,96: POKE 792,96: PRINT "SOUND OFF"
7061  FOR P = 0 TO 500 + EP: NEXT P: HOME : GOTO 20
7080  POKE 778,164: POKE 792,164: PRINT "SOUND ON": GOTO 7061

7100 M = 8: N = 9: FOR I = 7 TO 0 STEP -1: M = M*(DW$(A) <> AP$(A,I)) + I*(DW$(A) = AP$(A,I)): N = N*(DV$(A) <> AP$(A,I)) + I*(DV$(A) = AP$(A,I)): NEXT
7110 DW$(A) = AP$(A,M): DR%(A,0) = M: DV$(A) = AP$(A,N): DR%(A,1) = N: RETURN

7400  HOME: VTAB 21: GOSUB 140: M = PEEK(6) + 1: A = PEEK(7)
7410  IF  A > 13 THEN 7500
7420  IF  A > 8 THEN 7600
7430  GOSUB 370: PRINT "A ";GO$;" SAYS- HELLO! ";
7440  GET A$: HOME : GOTO 100

7500  IF WX% <> 0 OR LEFT$(T$,6)="CASTLE" THEN 7599
7510  ONERR GOTO 9250
7530  ON M GOTO 7700,7599,7702,7704,7599,7599,7599,7706,7708,7710,7599,7599,7599,7599,7710
7599  POKE 216,0: HOME: VTAB 21: PRINT "NOBODY HERE! ";: GOTO 199

7600  CALL 7850: PRINT " ";: A = PEEK(30)
7610  ONERR GOTO 9250
7620  ON A GOTO 7711,7712,7713,7714
7630  POKE 216,0:  ON A-4 GOTO 7720,7730,7740,7750,7760,7770
7699  GET A$: HOME : GOTO 100

7700  PRINT : PRINT CHR$(4);"BLOAD ";BP$;"FOOD": GOTO 8910
7702  PRINT : PRINT CHR$(4);"BLOAD ";BP$;"PUB": GOTO 8900
7704  PRINT : PRINT CHR$(4);"BLOAD ";BP$;"SHIPYARD": GOTO 8910
7706  PRINT : PRINT CHR$(4);"BLOAD ";BP$;"LIBRARY": GOTO 8900
7708  PRINT : PRINT CHR$(4);"BLOAD ";BP$;"TEMPLE": GOTO 8900
7710  PRINT : PRINT CHR$(4);"BLOAD ";BP$;"WEAPARM": GOTO 8900

7711  PRINT : PRINT CHR$(4);"BLOAD ";BP$;"SAGE": GOTO 8910
7712  PRINT : PRINT CHR$(4);"BLOAD ";BP$;"ARCHWIZ": GOTO 8910
7713  PRINT : PRINT CHR$(4);"BLOAD ";BP$;"ALCHEMIST": GOTO 8900
7714  IF FG%(5)=32 THEN POKE 216,0: PRINT "VANQUISHER! ";: GOTO 7699
7715  PRINT : PRINT CHR$(4);"BLOAD ";BP$;"BARON": GOTO 8900

7720  PRINT: GOSUB 7000: IF B$ = "GEM OF ENLIGHTENMENT" THEN 7722
7721  HOME : VTAB 21: PRINT "BE OFF! ";: GOTO 199
7722  HOME : VTAB 21: PRINT "2000 GOLD, TAKE IT OR LEAVE IT (Y/N) ";: GET A$
7724  IF A$ = "N" THEN 7721
7725  C = 2000: IF AD% < C THEN 7999
7726  A = 0: GOSUB 4600: GOTO 2040

7730  GET A$:B$ = "RED " + "PRISM": C = 0: GOTO 7726
7740  GET A$:B$ = "CLEAR " + "PRISM": C = 0: GOTO 7726

7750  PRINT : GOSUB 7000
7753  IF B$ = "YFJ" THEN 7755
7754  HOME : VTAB 21: PRINT "WRONG! ";: GOTO 199
7755 XT = XT + 6: HOME : GOTO 100

7760  PRINT : GOSUB 7000: IF  LEFT$ (B$,5) <> "SOULS" THEN 7721
7761  HOME : VTAB 21: PRINT "-SUMMONING PHANTOM SHIP-": GOSUB 30000: YT = YT + 1: N=11: GOSUB 141: YT = YT - 1: N=14: GOSUB 142: HOME : GOTO 100

7770  PRINT : GOSUB 7000: IF B$ = "SROTCES EERF" THEN 7772
7771  HOME : VTAB 21: PRINT "BE OFF, FOOLISH MORTAL! ";: GOTO 199
7772  HOME: PG$ = "DUN"+"GEON": R%=2: GOTO 8040

7999 HOME : VTAB 21: PRINT "YOU DON'T HAVE THE GOLD! ";: GOTO 199

8000 HOME : VTAB 21: IF FO%=32 AND FL%=0 THEN FL%=32
8010  PG$ = "OUT"+"SIDE" : R%=1
8040  POKE 216,0: POKE 4096,0: POKE 103,1: POKE 104,16: GOTO 10

8070  HOME: VTAB 21: PRINT "NOT HERE. ";: GOTO 199

8900 POKE 216,0: POKE 2048,0: POKE 103,1: POKE 104,8: GOTO 8900
8910 POKE 216,0: POKE 2816,0: POKE 103,1: POKE 104,11: GOTO 8910

9250  HOME: VTAB 21: PRINT "PLEASE RESTORE DISK- ";: GET A$: CALL 768: PRINT: RESUME

13000  M = 40: FOR I = 0 TO 3: IF DX$(I) = "PROTECTION 1" THEN M = 0
13010  NEXT
13020  FOR I = 0 TO 3: AB%(I) = AB%(I) - M: IF AB%(I) < 1 THEN AB%(I) = 0
13030  NEXT
13040  IF M > 0 THEN GOSUB 30080: GOSUB 40000: HOME: FS% = -ABS(FS%): IF FS% > -2 THEN FS% = -2
13049  GOTO 20

25000  HOME: IF L=0 THEN 100
25001  ONERR GOTO 9250
25002  PRINT: PRINT CHR$(4);"BLOAD ";RB$;"TWNSPS,A$800": POKE 216,0: GOTO 100

30000  POKE 6,64: POKE 7,2: POKE 8,100: POKE 9,1: POKE 30,2: CALL 806: RETURN
30050  POKE 6,50: POKE 7,25: POKE 8,1: POKE 9,1: POKE 30,1: CALL 852: GET A$: RETURN
30060  POKE 6,44: POKE 7,2: POKE 8,30: POKE 9,7: POKE 30,2: CALL 806: RETURN
30070  POKE 6,64: POKE 7,2: POKE 8,5: POKE 9,1: POKE 30,2: CALL 806: RETURN
30080  POKE 6,50: POKE 7,45: POKE 8,10: POKE 9,5: POKE 30,1: CALL 806: RETURN
30090  POKE 6,220: POKE 7,2: POKE 8,100: POKE 9,1: POKE 30,2: CALL 806: RETURN

30100  FOR P = 0 TO 500+EP: NEXT P: RETURN

40000 M = 0: FOR I = 0 TO 3:M = M + AB%(I): NEXT
40001  IF M > 0 THEN  RETURN
50000  HOME : VTAB 21: PRINT "YOUR PARTY IS ELIMINATED!"
50001  GOTO 50001
