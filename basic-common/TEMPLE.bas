...
33 TEXT : HOME : PRINT "WELCOME TO THE TEMPLE, ";NM$(A)
34 PRINT : PRINT "ONE MOMENT WHILE I SUMMON THE GODS...": FOR P = 0 TO EP: NEXT P
35 P = 0: M =  FRE (0)
36 FOR I = 0 TO 15:AX$(A,I) = "": NEXT
37 M =  FRE (0):WT% =  INT (ZA%(A) / 1000) * 100 + 100:WT% = WT% + (WT% * (AH%(A) / 100))
38 IF CL$(A) = "PALADIN" THEN WT% =  INT (WT% / 2)
39 IF CL$(A) = "MONK" THEN WT% =  INT (WT% / 4)
...
50 AX$(A,P) =  LEFT$ (A$ + " ", LEN (A$)): IF P=15 OR WT%<40 THEN 25000
...
56 IF CL$(A) <> "CLERIC" AND CL$(A) <> "MONK" AND CL$(A) <> "PALADIN" THEN A$ = "NOT A CLERIC, MONK, OR PALADIN. ": GOTO 99
57 C = (10 +  INT (ZA%(A) / 1000) * 8) * (1 - AK%(A) / 100)
58 PRINT: PRINT "TEMPLE COSTS ";INT(C);" GOLD, OK? (Y/N) ";: GET A$: IF A$ <> "Y" THEN POP: GOTO 25001
59 AD% = AD% - C
