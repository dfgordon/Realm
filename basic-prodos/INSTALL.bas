...
4  DIM PN$(63): DIM CAP(63): DIM PLNGTH(63): A0GTVOL = 16451: A1GTCAP = 16460: A2COPY = 16541
5  A3STRBUF = 20480: A4ERR = 6: A5DBG = 8
6  A6SRCPRE = 20736: A7SRCPTH = 20800: A8DSTPRE = 20864: A9DSTPTH = 20928
7  DEF FN LO(X) = X - 16 * INT(X/16)
8  PRINT: PRINT CHR$(4);"BLOAD /REALM.INSTALL1/DISKLIB"
9  GOTO 1000

10  REM GET VOLUMES AND CAPACITIES
30  CALL A0GTVOL: ADDR = A3STRBUF: COUNT = 0
40  IF PEEK(A4ERR)>0 THEN PRINT "PRODOS ERROR ";PEEK(A4ERR): GOTO 9999
60  PN$(COUNT) = "": L = FN LO(PEEK(ADDR)): IF L=0 OR COUNT>8 THEN 90
70  FOR J = 1 TO L: PN$(COUNT) = PN$(COUNT) + CHR$(PEEK(ADDR+J)): NEXT
80  ADDR = ADDR + 16: COUNT = COUNT + 1: GOTO 60
90  IF COUNT=0 THEN PRINT "NO VOLUMES FOUND": GOTO 9999
91  FOR J = 0 TO COUNT-1: VO$ = "/" + PN$(J): L = LEN(VO$): POKE A3STRBUF,L
92  FOR I = 1 TO L: POKE A3STRBUF+I,ASC(MID$(VO$,I,1)): NEXT: CALL A1GTCAP: CAP(J) = PEEK(6) + 256*PEEK(7)
93  PN$(J) = PN$(J) + " (" + STR$(CAP(J)) + ")": NEXT: RETURN
...
190  REM COPY PTH$ TO ADDRESS P WITH LENGTH BYTE
191  POKE P,LEN(PTH$): FOR I = 1 TO LEN(PTH$): POKE P+I,ASC(MID$(PTH$,I,1)): NEXT
192  RETURN

1000  HOME: PRINT "REALM HD INSTALLER V1.5.0": POKE 34,2
1001  GOSUB 10: REM ENDS IF NO VOLUMES
1010  L = 3: B$ = CHR$(13) + CHR$(9): W$ = "RET = SELECT VOLUME" + CHR$(13) + "TAB = RESCAN": I1 = 0: I2 = COUNT-1: GOSUB 150
1011  IF A$ = CHR$(9) THEN HOME: GOTO 1001
1020  IF CAP(M) < 1000 THEN PRINT: PRINT "NOT ENOUGH ROOM ON VOLUME ";: GET A$: HOME: GOTO 1010
1030  PTH$ = "/" + LEFT$(PN$(M),LEN(PN$(M))-LEN(STR$(CAP(M)))-3)
1040  VO$ = PTH$: PLNGTH(0) = LEN(VO$): DEPTH = 0

1050  PAGE = 0
1060  GOSUB 170
1070  HOME: PRINT: PRINT "CURRENT PATH:": PRINT PTH$: B$ = " ": L = 6
...
1200  HOME: DST$ = PTH$: PRINT "INSTALLING TO": PRINT DST$ + "/REALM"
1201  PRINT "PROCEED";: GOSUB 160: PRINT: IF A$="N" THEN PRINT "ABORTED.": GOTO 9999
1210  ONERR GOTO 6020
1220  PRINT CHR$(4);"PREFIX ";DST$
...
1320  FOR FCOUNT = 1 TO FILES: READ A$
1321  P = A6SRCPRE: PTH$ = "/REALM.INSTALL" + STR$(DSK) + "/ITEMS": GOSUB 190
1330  P = A7SRCPTH: PTH$ = A$: GOSUB 190
1331  P = A8DSTPRE: PTH$ = DST$: GOSUB 190
1340  P = A9DSTPTH: PTH$ = SUBDIR$ + "/" + A$: GOSUB 190
1341  PRINT CHR$(4);"VERIFY /REALM.INSTALL";DSK
1342  A$ = DST$+"/"+PTH$: IF LEN(A$)>39 THEN PRINT "...";RIGHT$(A$,36)
1343  IF LEN(A$)<40 THEN PRINT A$ 
1350  POKE A5DBG,0: CALL A2COPY: IF PEEK(A4ERR)>0 THEN PRINT "PRODOS ERROR ";PEEK(A4ERR): GOTO 9999
1360  NEXT: GOTO 1270
1370  PRINT: PRINT "INSTALLATION FINISHED.": GOTO 9999

6000  PRINT: PRINT "UNRECOVERABLE ERROR ";PEEK(222);" ON LINE ";PEEK(218)+PEEK(219)*256
6010  GOTO 9999

6020  PRINT "ERROR ";PEEK(222);" WHILE CREATING DIRECTORIES": GOTO 9999

6030  IF PEEK(222) = 42 THEN 1370: REM OUT OF DATA
6031  IF PEEK(222) = 6 OR PEEK(222) = 7 THEN 6050: REM PATH NOT FOUND 
6040  GOTO 6000
6050  PRINT: PRINT "INSERT INSTALL";DSK;" THEN PRESS ANY KEY- ";: GET A$: PRINT: RESUME

8000  DATA REALM, disk=1, files=3
8001  DATA START.REALM,TITLE.PIC,DD

8008  DATA REALM/BIN, disk=1, files=6
8009  DATA MAP.INTRP,SOUND,SDP.INTRP,DNGNSPS,OUTSPS,TWNSPS
