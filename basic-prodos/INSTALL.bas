...
1001  POKE 34,0: HOME: PRINT "REALM HD INSTALLER V1.5.1": POKE 34,2
...
1204  IF UPGD = 1 THEN 1260: REM ALREADY CONFIRMED UPGRADE SO GO AHEAD
1205  UPGD = 1: GOSUB 200: GOSUB 160: PRINT: IF A$="N" THEN GOTO 1000
1206  GOTO 1260
1210  PRINT "PROCEED";: GOSUB 160: PRINT: IF A$="N" THEN GOTO 1000: REM CONFIRM FRESH INSTALL
...
1325  ONERR GOTO 6060
1326  PRINT CHR$(4);"VERIFY ";DST$;"/";SUBDIR$
1329  ONERR GOTO 6000
...
6000  REM UNRECOVERABLE
6001  PRINT: PRINT CHR$(7);"UNRECOVERABLE ERROR ";PEEK(222);" ON LINE ";PEEK(218)+PEEK(219)*256: GOTO 9999
6010  REM HANDLE EXISTING
6011  IF (PEEK(222) = 6 OR PEEK(222)=7) AND UPGD = 0 THEN CALL -3288: GOTO 1210
6012  GOTO 6000  
6020  REM OUT OF DATA MEANS WE ARE DONE
6021  IF PEEK(222) = 42 THEN 1370
6030  GOTO 6000
6040  REM SRC DISK MISSING
6041  PRINT: PRINT "INSERT INSTALL";DSK;" THEN PRESS ANY KEY- ";: GET A$: PRINT: RESUME
6050  REM DST DISK MISSING
6051  PRINT: PRINT "MOUNT ";VO$;" THEN PRESS ANY KEY- ";:GET A$: PRINT: RESUME
6060  REM SUBDIR MISSING
6061  CALL -3288: PRINT: PRINT SUBDIR$;" IS MISSING.": PRINT "CREATE IT AND CONTINUE";: GOSUB 160: IF A$="N" THEN GOTO 1000
6062  PRINT: PRINT CHR$(4);"CREATE ";DST$;"/";SUBDIR$: PRINT: RESUME
6070  REM CONFIG MISSING
6071  CALL -3288: PRINT: PRINT "CONFIG FILE IS MISSING.": PRINT "CREATE IT AND CONTINUE";: GOSUB 160: IF A$="N" THEN GOTO 1000
6072  PRINT: &("COPY",P1$,P2$,P3$,P4$): PRINT: RESUME
